...
--DECLARE @ausweisNr int, @vname varchar(max), @nname varchar(max);


USE[Bibliothek]
GO
CREATE PROCEDURE sp_TestAnlegenLoeschenAutor @ausweisNr int, @vname varchar(max), @nname varchar(max)
AS
DECLARE @anzVor int = (select count(*) from Autoren)

PRINT 'Versuche einen Autor anzulegen'
EXEC sp_LegeAutorAn @ausweisNr, @vname, @nname
...
BEGIN
	...
	BEGIN
		...
		EXEC sp_LoescheAutor @ausweisNr, @autorId 
...
EXEC sp_TestAnlegenLoeschenAutor 2, 'test autor', 'test autor nachname'