...
USE [Bibliothek]
GO
CREATE PROCEDURE sp_TestNutzerAnlegenLoeschen @ausweisNr int, @vname nvarchar(max), @nname nvarchar(max), @gdatum date, @pw varchar(16),
						@mitarbeiterrecht bit, @kontostand smallmoney
AS
DECLARE @anzahlNutzerNach int, @anzahlNutzerVor int = (select count(*) from nutzer);	
	PRINT 'Versuche neuen Nutzer anzulegen'
EXEC sp_LegeNutzerAn @ausweisNr, @mitarbeiterrecht, @vname, @nname, @gdatum, @kontostand, @pw
...
EXEC sp_TestNutzerAnlegenLoeschen 2, '#_test_vorname_#', '#_test_nachname_#', '01.01.1960', '#x_test_x#', 0, 0